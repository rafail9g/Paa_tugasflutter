...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  void initState() {
    super.initState();
    // Selalu load fresh dari API agar status pembayaran terkini
    _loadBooking();
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            b.id
                    ?.substring(b.id!.length > 8 ? b.id!.length - 8 : 0)
                    .toUpperCase() ??
                '-',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${car.brand}${car.type != null ? ' • ${car.type!.toUpperCase()}' : ''}',
  ...
  Widget _buildPaymentInfo() {
    ...
    // Tentukan pesan info berdasarkan status pembayaran
    Widget? infoBox;

    if (b.paymentStatus == 'unpaid' && b.status != 'cancelled') {
      // Belum ada payment sama sekali
      infoBox = _buildInfoBox(
        icon: Icons.info_outline,
        iconColor: Colors.amber.shade700,
        bgColor: Colors.amber.shade50,
        borderColor: Colors.amber.shade200,
        text: 'Selesaikan pembayaran untuk mengaktifkan pemesanan Anda.',
      ...
      infoBox = _buildInfoBox(
        ...
        text:
            'Pembayaran Anda sedang diverifikasi oleh admin. Mohon tunggu konfirmasi.',
      ...
      infoBox = _buildInfoBox(
        ...
        text: 'Pembayaran telah diverifikasi. Pemesanan aktif.',
      ...
      Column(
        ...
        children: [
          ...
          if (b.paymentMethod != null) ...[
            ...
            _buildInfoRow(
              'Metode',
              _getPaymentMethodLabel(b.paymentMethod!),
              Icons.account_balance_wallet,
            ),
          ],
          if (infoBox != null) ...[const SizedBox(height: 12), infoBox],
        ],
      ),
    );
  }
  ...
  Widget _buildActions() {
    ...
    // Tampilkan tombol bayar hanya jika belum ada payment sama sekali (unpaid)
    final canPay = b.paymentStatus == 'unpaid' && b.status != 'cancelled';

    // Jika sudah kirim payment tapi belum terverifikasi (pending),
    // sembunyikan tombol bayar — sudah dalam proses verifikasi
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () async {
                final result = await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => PaymentScreen(booking: b),
                  ),
                );
                // Refresh data setelah kembali dari halaman pembayaran
